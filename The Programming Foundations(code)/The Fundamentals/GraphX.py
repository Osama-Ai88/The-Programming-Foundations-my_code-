...
class Graph(object):
    ...
    def MST(self):
        ...
        F = open("GraphtextW.txt", 'r')
        
        Number_Of_Vertices = int(F.readline())
        
        edges=[[] for x in range(Number_Of_Vertices)]
        
        for t in F.readline():
            
            edge = tuple(map(int,t.split(" ")))
        ...
        while len(UV)<Number_Of_Vertices:
            ...
            for v in UV:
                ...
                while(len(edges[v]) > 0 and edges[v][0][dest] in UV ):
                    
                    heapq.heappop(edges[v])
                
                if len(edges[v] == 0 ):
                    continue
                
                if(len(edges[verticesSmall])==0) or edges[v][0][cost]< edges[verticesSmall][0][cost]:
    ...
    def Draw_MST(self):
        ...
        GraphMST= nx.read_weighted_edgelist("GraphtextW.txt",delimiter=" ")
        ...
        nx.draw(GraphMST,with_labels=True,node_size=600, node_color="y")
